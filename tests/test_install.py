...
class InstallTests(unittest.TestCase):
    ...
    def test_install_agd_writes_shell_function_and_completion(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            wrapper_dir = Path(temp_dir) / "bin"
            wrapper_dir.mkdir()
            completion_dir = Path(temp_dir) / "zsh"
            shell_dir = Path(temp_dir) / "shell"
            old_wrapper = wrapper_dir / "agd"
            old_wrapper.write_text(CHIYO.wrapper_script("agd"), encoding="utf-8")
            config_path = os.path.join(temp_dir, "config.toml")
            Path(config_path).write_text(
                "\n".join(
                    [
            ...
            Path(tools_config_path).write_text("", encoding="utf-8")

            with mock.patch.object(CHIYO, "CONFIG_PATH", config_path):
                with mock.patch.object(CHIYO, "TOOLS_CONFIG_PATH", tools_config_path):
                    lines = CHIYO.install_tool_lines("agd")

            shell_file = shell_dir / "agd.zsh"
            completion = completion_dir / "_agd"
            wrapper = wrapper_dir / "agd"

            self.assertIn(f"installed agd shell: {shell_file}", lines)
            self.assertIn(f"installed _agd: {completion}", lines)
            self.assertIn(f"removed old agd wrapper: {old_wrapper}", lines)
            self.assertEqual(
                shell_file.read_text(encoding="utf-8"),
                CHIYO.shell_function_script("agd"),
            )
            self.assertEqual(
                completion.read_text(encoding="utf-8"),
                CHIYO.completion_script("agd"),
            )
            self.assertFalse(wrapper.exists())
